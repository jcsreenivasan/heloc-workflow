...
export function StepThree({ data, onChange, onNext }: StepThreeProps) {
  ...
  useEffect(() => {
    if (showQ8) setTimeout(() => q8Ref.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 350);
  }, [showQ8]);
  ...
  return (
    <div className="px-5 py-6">
      ...
      <div className="pb-7">
        ...
        {!showQ8 && <NextBtn onClick={() => setShowQ8(true)} />}
      </div>

      {/* Q8: Employment status */}
      <AnimatePresence>
        {showQ8 && (
          <motion.div ref={q8Ref} key="q8" {...reveal} className="border-t border-gray-100 pt-7">
            <h2 className="text-lg font-black text-gray-900 mb-4">
